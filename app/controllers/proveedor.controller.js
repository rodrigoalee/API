...
const Proveedor = db.Proveedor; // Asegúrate de que el nombre sea correcto
...
exports.updateById = async (req, res) => {
    try {
        ...
        if (!proveedor) {
            ...
            if (!result) {
                res.status(500).json({
                    message: "No se puede actualizar el proveedor con id = " + req.params.id_proveedor,
                    error: "No se pudo actualizar el proveedor",
                });
            }

            res.status(200).json({
                message: "Actualización exitosa del proveedor con id = " + proveedorId,
...
exports.deleteById = async (req, res) => {
    try {
        ...
        res.status(500).json({
            message: "No se puede eliminar el proveedor con id = " + req.params.id_proveedor,
            error: error.message,
        });
    }
};
